...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="",budget_cat="", limit=5):
        """
        ...
        if budget_cat == None:
            budget_cat = "''"
        #req = requests.get(base_url + "search?q=" + str(query) + "&cft=" + str(budget_cat) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon='" + str(longitude) + "'&cuisines=" + str(cuisines) + "&sort=rating", headers=headers)
        req = requests.get(base_url + "search?q='" + str(query) + "'&cft=" + str(budget_cat) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines) + "&sort=rating", headers=headers)
